...
$id = $_GET['id']; // Asumiendo que el ID del delivery viene por la URL
$delivery = $dc->obtenerDeliveryDetalladoPorId($id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $datos = [
        ...
        'tipo_contraentrega' => $_POST['tipo_contraentrega']
    ];
...
<div class="d-flex flex-column justify-content-center align-items-center w-100 h-100">
    <div class="container m-10 rounded-4 shadow-sm col-lg-6 col-md-12 p-4" style="background-color: white;">    
        <h2 class="mb-4 fs-5">Editar delivery</h2>
            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="step1" role="tabpanel" aria-labelledby="step1-tab">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="descripcion" name="descripcion" value="<?php echo $delivery['delivery']->getDescripcion(); ?>" required>
                    </div>
                    <div class="mb-3 d-flex flex-column">
                        <label for="descripcion">¿Debemos cobrar al destinatario?</label>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="tipo_contraentrega" id="no_cobrar" value="no_cobrar" <?php echo ($delivery['contraentrega']->getCostoDelivery() == 0 && $delivery['contraentrega']->getCostoPedido() == 0) ? 'checked' : ''; ?>>
                            <label class="form-check-label" for="no_cobrar">No cobrar</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="tipo_contraentrega" id="solo_pedido" value="solo_pedido" <?php echo ($delivery['contraentrega']->getCostoDelivery() == 0 && $delivery['contraentrega']->getCostoPedido() > 0) ? 'checked' : ''; ?>>
                            <label class="form-check-label" for="solo_pedido">Solo valor del pedido</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="tipo_contraentrega" id="solo_delivery" value="solo_delivery" <?php echo ($delivery['contraentrega']->getCostoDelivery() > 0 && $delivery['contraentrega']->getCostoPedido() == 0) ? 'checked' : ''; ?>>
                            <label class="form-check-label" for="solo_delivery">Solo delivery</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="tipo_contraentrega" id="cobrar_ambos" value="cobrar_ambos" <?php echo ($delivery['contraentrega']->getCostoDelivery() > 0 && $delivery['contraentrega']->getCostoPedido() > 0) ? 'checked' : ''; ?>>
                            <label class="form-check-label" for="cobrar_ambos">Cobrar ambos</label>
                        </div>
                    </div>
                    <div class="mb-3 contraentrega-fields">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.1" placeholder="0.00" class="form-control" id="costo_delivery" name="costo_delivery" value="<?php echo $delivery['contraentrega']->getCostoDelivery(); ?>" required>
                        </div>
                    </div>
                    <div class="mb-3 contraentrega-fields">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">S/.</span>
                            <input type="number" step="0.1" placeholder="0.00" class="form-control" id="costo_pedido" name="costo_pedido" value="<?php echo $delivery['contraentrega']->getCostoPedido(); ?>" required>
                        </div>
                    </div>
                ...
                <div class="tab-pane fade" id="step2" role="tabpanel" aria-labelledby="step2-tab">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="direccion_recojo" name="direccion_recojo" value="<?php echo $delivery['recojo']->getDireccion(); ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="fecha_recojo">Cuándo lo recogemos</label>
                        <input type="date" class="form-control" id="fecha_recojo" name="fecha_recojo" value="<?php echo $delivery['recojo']->getFecha(); ?>">
                    </div>
                    <div class="mb-3">
                        <label for="hora_recojo">A qué hora lo recogemos (considere 5min de retraso)</label>
                        <input type="time" class="form-control" id="hora_recojo" name="hora_recojo" value="<?php echo $delivery['recojo']->getHora(); ?>">
                    </div>
                    <div class="d-flex justify-content-end">
                ...
                <div class="tab-pane fade" id="step3" role="tabpanel" aria-labelledby="step3-tab">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="direccion_entrega" name="direccion_entrega" value="<?php echo $delivery['entrega']->getDireccion(); ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="fecha_entrega">Fecha</label>
                        <input type="date" class="form-control" id="fecha_entrega" name="fecha_entrega" value="<?php echo $delivery['entrega']->getFecha(); ?>">
                    </div>
                    <div class="mb-3">
                        <label for="hora_entrega">Hora</label>
                        <input type="time" class="form-control" id="hora_entrega" name="hora_entrega" value="<?php echo $delivery['entrega']->getHora(); ?>">
                    </div>
                    <div class="d-flex justify-content-end">
                        <button type="button" class="btn btn-outline-secondary prev-step me-2">Anterior</button>
                        <button type="button" class="btn btn-outline-primary next-step">Siguiente</button>
                ...
                <div class="tab-pane fade" id="step4" role="tabpanel" aria-labelledby="step4-tab">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="dni_destinatario" name="dni_destinatario" value="<?php echo $delivery['destinatario']->getDni(); ?>">
                    </div>
                    <div class="mb-3">
                        <label for="nombres_destinatario">Nombres</label>
                        <input type="text" class="form-control" id="nombres_destinatario" name="nombres_destinatario" value="<?php echo $delivery['destinatario']->getNombres(); ?>">
                    </div>
                    <div class="mb-3">
                        <label for="apellidos_destinatario">Apellidos</label>
                        <input type="text" class="form-control" id="apellidos_destinatario" name="apellidos_destinatario" value="<?php echo $delivery['destinatario']->getApellidos(); ?>">
                    </div>
                    <div class="mb-3">
                        <label for="celular_destinatario">Celular</label>
                        <input type="tel" class="form-control" id="celular_destinatario" name="celular_destinatario" value="<?php echo $delivery['destinatario']->getCelular(); ?>" required>
                    </div>
                    <div class="d-flex justify-content-end">
                        <button type="button" class="btn btn-outline-secondary prev-step me-2">Anterior</button>
                        <button type="button" class="btn btn-outline-primary next-step">Siguiente</button>
                ...
                <div class="tab-pane fade" id="step5" role="tabpanel" aria-labelledby="step5-tab">
                    <div class="mb-3">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="monto_pago" id="tarifa_1" value="6" <?php echo ($delivery['pago']->getMonto() == 6) ? 'checked' : ''; ?>>
                            <label class="form-check-label text-secondary" for="tarifa_1">Cercado de Huánuco - S/. 6,00</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="monto_pago" id="tarifa_2" value="7" <?php echo ($delivery['pago']->getMonto() == 7) ? 'checked' : ''; ?>>
                            <label class="form-check-label text-secondary" for="tarifa_2">Urbano 1 (Amarilis) - S/. 7,00</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="monto_pago" id="tarifa_3" value="8" <?php echo ($delivery['pago']->getMonto() == 8) ? 'checked' : ''; ?>>
                            <label class="form-check-label text-secondary" for="tarifa_3">Urbano 2 (La Esperanza y Cayhuayna) - S/. 8,00</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="monto_pago" id="tarifa_4" value="10" <?php echo ($delivery['pago']->getMonto() == 10) ? 'checked' : ''; ?>>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" aria-label="Default select example" name="metodo_pago">
                            <option value="Yape / Plin" <?php echo ($delivery['pago']->getMetodo() == 'Yape / Plin') ? 'selected' : ''; ?>>Yape / Plin</option>
                            <option value="BCP" <?php echo ($delivery['pago']->getMetodo() == 'BCP') ? 'selected' : ''; ?>>Transferencia (BCP)</option>
                            <option value="Interbank" <?php echo ($delivery['pago']->getMetodo() == 'Interbank') ? 'selected' : ''; ?>>Transferencia (Interbank)</option>
                            <option value="BBVA" <?php echo ($delivery['pago']->getMetodo() == 'BBVA') ? 'selected' : ''; ?>>Transferencia (BBVA)</option>
                            <option value="Paypal" <?php echo ($delivery['pago']->getMetodo() == 'Paypal') ? 'selected' : ''; ?>>Paypal</option>
                        </select>                
                    </div>
    ...
    function showFields(option) {
        ...
        if (option === 'no_cobrar') {
            ...
            if (option === 'solo_pedido' || option === 'cobrar_ambos') {
                ...
                costoPedido.value = '<?php echo $delivery['contraentrega']->getCostoPedido(); ?>';
            } else {
                costoPedido.value = '0';
            }
            if (option === 'solo_delivery' || option === 'cobrar_ambos') {
                costoDelivery.parentNode.parentNode.style.display = 'block';
                costoDelivery.value = '<?php echo $delivery['contraentrega']->getCostoDelivery(); ?>';
            } else {
                costoDelivery.value = '0';
            }
        }
